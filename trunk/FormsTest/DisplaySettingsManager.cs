...
namespace MailClient.Common.UI
{
    public class DisplaySettingsManager
    {
        ...
        private static DisplaySettingsManager instance;
        ...
        private float osWideScaleFactor = 1.0f;

        public float OSWideScaleFactor
        {
            get
            {
                return osWideScaleFactor;
            }
        }
        private Point lastTouchPosition;
        private Point lastTouchStartPosition;

        #region DisplaySettingsManager public properties
        /// <summary>
        /// Returns the instance of DisplaySettingsManager
        /// </summary>
        public static DisplaySettingsManager Instance
        {
            get
            {
                if (UI.CommonUIUtils.DesignMode)
                {
                    DisplaySettingsManager.Initialize();
                }
                if (initialized)
                    return instance;
                else throw new InvalidOperationException("DisplaySettings hasn't been initialized.");

            }
        }

        public int Scale(int number)
        {
            return (int)Math.Round((float)number * scaleFactor);
        }
        public float Scale(float number)
        {
            return (float)Math.Round(number * scaleFactor);
        }
        ...
        public Point LastTouchStartPosition
        {
            get
            {
                return lastTouchStartPosition;
            }
        }
        #endregion

        private DisplaySettingsManager()
        {
            Microsoft.Win32.SystemEvents.DisplaySettingsChanged += SystemEvents_DisplaySettingsChanged;
            UpdateCurrentScaleFactors();
        }

        private void UpdateCurrentScaleFactors()
        {
            // FIXME: Mac
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
                using (Graphics g = Graphics.FromHwnd(IntPtr.Zero))
                {
                    ...
                    osWideScaleFactor = (float)PhysicalScreenHeight / (float)LogicalScreenHeight;
                    g.ReleaseHdc(desktop);
                }

                scaleFactor = (float)((float)getDPI().Width / 96.0f);
            }
        }
        ...
        {
            lock (typeof(DisplaySettingsManager))
            {
        ...
        public static void Uninitialize()
        {
            lock (typeof(DisplaySettingsManager))
            {
                ...
                /* workaround: don't set the instance to null, so it won't crash in specific exit scenarios
                initialized = false;
                instance = null;
                 */
            }
        ...
        private Size getDPI()
        {
            ...
            using (Graphics graphics = Graphics.FromHwnd(IntPtr.Zero))
                return new Size((int)graphics.DpiX, (int)graphics.DpiY);
        }



        private bool touchInputEnabled = false;
        private Dictionary<string, int> defaultToolStripItemsHeight = new Dictionary<string, int>();
        private Dictionary<string, int> defaultToolStripItemsPadding = new Dictionary<string, int>();
        private Dictionary<string, int> defaultMenuStripItemsHeight = new Dictionary<string, int>();

        public bool TouchInputEnabled
        {
            get { return touchInputEnabled; }
            set
            {
                if (touchInputEnabled != value)
                {
                    if (TouchInputEnabledChanging != null)
                        TouchInputEnabledChanging(this, EventArgs.Empty);

                    touchInputEnabled = value;

                    if (TouchInputEnabledChanged != null)
                        TouchInputEnabledChanged(this, EventArgs.Empty);
                }
            }
        }

        public int TouchScale(int value)
        {
            if (touchInputEnabled)
                return (int)(value * TouchInputScaleFactor);
            return value;
        }

        public int TouchScaleSmall(int value)
        {
            if (touchInputEnabled)
                return (int)(value * TouchInputScaleFactorSmall);
            return value;
        }

        public Size TouchScale(Size size)
        {
            if (touchInputEnabled)
                return new Size((int)(size.Width * TouchInputScaleFactor), (int)(size.Height * TouchInputScaleFactor));
            return size;
        }

        public float TouchInputScaleFactor
        {
            get { return 2.0f; }
        }

        public float TouchInputScaleFactorSmall
        {
            get { return 1.6f; }
        }

        public event EventHandler TouchInputEnabledChanged;
        public event EventHandler TouchInputEnabledChanging;

        public bool TouchInputSupport
        {
            get
            {
                try
                {
                    int maximumTouches = GetSystemMetrics(SystemMetric.SM_MAXIMUMTOUCHES);
                    if (maximumTouches > 1)
                        return true;
                    else
                        return false;
        ...
        public void ScaleItemsInToolStrip(ToolStrip toolStrip)
        {
            ...
            string toolStripPath = getControlPath(toolStrip);

            if (!defaultToolStripItemsHeight.TryGetValue(toolStripPath, out originalHeight))
            {
                foreach (ToolStripItem item in toolStrip.Items)
                {
                    originalHeight = item.Height;
                    defaultToolStripItemsHeight.Add(toolStripPath, originalHeight);
                    break;
                }
            }

            if (!defaultToolStripItemsPadding.TryGetValue(toolStripPath, out originalPadding))
            {
                foreach (ToolStripItem item in toolStrip.Items)
                {
                    if (!(item is ToolStripSeparator))
                    {
                        originalPadding = item.Padding.Left;
                        defaultToolStripItemsPadding.Add(toolStripPath, originalPadding);
                        break;
                    }
                }
            }
            ...
            if (originalHeight > 0)
            {
                foreach (ToolStripItem item in toolStrip.Items)
                {
                    ...
                    {
                        item.AutoSize = false;
                        item.Height = TouchScaleSmall(originalHeight);
                    }
                    else
                    {
                        int padding = TouchScaleSmall(originalPadding);
                        if (TouchInputEnabled && padding == 0)
                            padding = 2;
                        item.Padding = new Padding(padding, item.Padding.Top, padding, item.Padding.Bottom);
                    }
                }
            }
        ...
        public void ScaleItemsInMenuStrip(MenuStrip menuStrip)
        {
            int originalHeight = 0;

            menuStrip.SuspendLayout();

            string menuStripPath = getControlPath(menuStrip);

            if (!defaultMenuStripItemsHeight.TryGetValue(menuStripPath, out originalHeight))
            {
                foreach (ToolStripItem item in menuStrip.Items)
                {
                    originalHeight = item.Height;
                    defaultMenuStripItemsHeight.Add(menuStripPath, originalHeight);
                    break;
                }
            }
            ...
            if (originalHeight > 0)
            {
                foreach (ToolStripItem item in menuStrip.Items)
                {
                    int newHeight = MailClient.Common.UI.DisplaySettingsManager.Instance.TouchScaleSmall(originalHeight);
                    int diff = newHeight - item.Height;
                    item.Padding = new Padding(item.Padding.Left, item.Padding.Top + diff / 2, item.Padding.Right, item.Padding.Bottom + diff / 2);
                }
            }
        ...
        private string getControlPath(Control control)
        {
            if (control.Parent != null)
                return string.Format("{0}.{1}", getControlPath(control.Parent), control.Name);

            return control.Name;
        }
        public void SaveLastTouchPosition(Point p)
        {
            this.lastTouchPosition = p;
        }
        public void ResetLastTouchPosition()
        {
            this.lastTouchPosition = new Point(int.MinValue, int.MinValue);
        }
        public void SaveLastTouchStartPosition(Point p)
        {
            this.lastTouchStartPosition = p;
        }
        public void ResetLastTouchStartPosition()
        {
            this.lastTouchStartPosition = new Point(int.MinValue, int.MinValue);
        }
